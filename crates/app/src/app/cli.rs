...
pub struct CaptureArgs {
    ...
    #[arg(long, default_value_t = false, help = "Enable cursor smear (motion trail)")]
    pub cursor_smear: bool,
    #[arg(long = "cursor-smooth-spring-k", default_value_t = 120.0, help = "Cursor spring stiffness (only used with --cursor-smooth)")]
    pub cursor_spring_k: f32,
    #[arg(long = "cursor-smooth-spring-d", default_value_t = 18.0, help = "Cursor spring damping (only used with --cursor-smooth)")]
    pub cursor_spring_d: f32,
    #[arg(long = "cursor-smooth-max-speed", default_value_t = 3000.0, help = "Cursor spring max speed (px/s, only used with --cursor-smooth)")]
    pub cursor_max_speed: f32,
    #[arg(long = "cursor-smooth-snap-px", default_value_t = 0.0, help = "Snap to target when distance exceeds this (px, only used with --cursor-smooth)")]
    pub cursor_snap_px: f32,
    #[arg(long = "cursor-smooth-ms", default_value_t = 12.0, help = "Cursor target smoothing time constant (ms, only used with --cursor-smooth)")]
    pub cursor_smooth_ms: f32,
    #[arg(long = "cursor-smooth-deadzone-px", default_value_t = 0.5, help = "Ignore jitter under this distance (px, only used with --cursor-smooth)")]
    pub cursor_deadzone_px: f32,
    #[arg(long = "cursor-smear-speed-threshold", default_value_t = 100.0, help = "Minimum cursor speed (px/s) required to emit smear trail")]
    pub cursor_smear_speed_threshold: f32,
    #[arg(long = "cursor-smear-shutter-scale", default_value_t = 4.0, help = "Smear shutter multiplier in frame-time units")]
    pub cursor_smear_shutter_scale: f32,
    #[arg(long = "cursor-smear-min-len", default_value_t = 4.0, help = "Minimum smear length in pixels once smear is active")]
    pub cursor_smear_min_len: f32,
    #[arg(long = "cursor-smear-max-len", default_value_t = 220.0, help = "Maximum smear length in pixels")]
    pub cursor_smear_max_len: f32,
    #[arg(long = "cursor-smear-taps", default_value_t = 8, help = "Number of additional smear taps (max 8)")]
    pub cursor_smear_taps: u32,
    #[arg(long = "cursor-smear-alpha-exp", default_value_t = 1.1, help = "Smear alpha falloff exponent")]
